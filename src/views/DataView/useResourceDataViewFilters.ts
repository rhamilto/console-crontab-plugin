...
import { useExactSearch } from "./useExactSearch";
import { K8sResourceCommon } from "@openshift-console/dynamic-plugin-sdk";
import { exactMatch, fuzzyCaseInsensitive } from "./table-filters";
  ...
  const filteredData = React.useMemo(
    () =>
      data.filter((resource) => {
        ...
        const matchesLabels =
          !filters.labels ||
          filterLabelsArray.every((label) => resourceLabels.includes(label));

        return (
          matchesName &&
          matchesLabels &&
          (matchesAdditionalFilters?.(resource, filters) ?? true)
        );
      }),
